<script>
  ...
  const selectTimeSlot = ({ start, end }) => {
    ...
    const generated = timeSlotsGen(
      start,
      end,
      DURATIONS_TIME[$newPollStore.duration]
    );
    const newTimesSlots = [];
    generated.forEach((t) => {
      if (
        $newPollStore.meetingSlots.find(
          (s) =>
            ...
            moment(t)
              ...
              .isBetween(moment(s.start), moment(s.end))
        )
      ) {
        return null;
      }
      ...
      newTimesSlots.push({
        start: new Date(t),
          ...
          moment(t).add(DURATIONS_TIME[$newPollStore.duration], "minutes")
        ),
      });
    });
  ...
  <div class="container">
    ...
    <div class="columns is-multiline is-mobile">
      ...
      <div class="column is-one-third-desktop is-full-mobile">
        <SelectedDatesTable
          bind:dates={$newPollStore.meetingSlots}
          meetingSlots={true}
        />
      </div>
      <div class="column is-full">
